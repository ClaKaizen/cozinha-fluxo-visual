...
import { ChevronLeft, ChevronRight, Clock, Users, Gauge, Activity, UserMinus, UserPlus, Info } from "lucide-react";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="border-l-4 border-l-primary">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">Inclui 20% de ineficiência</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-primary">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <Tooltip>
              ...
              <TooltipContent className="max-w-[260px] text-xs">
                Pessoas × 7.5h × (1 − 6.25% pausa) = {stats.pessoasPresentes} × 7.5 × 0.9375
              </TooltipContent>
            </Tooltip>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-display font-bold">{stats.capacidadeDoDia.toFixed(1)}h</div>
            <p className="text-xs text-muted-foreground mt-1">7h30 com coef. pausa 6.25%</p>
          </CardContent>
        </Card>
            ...
            {production.length === 0 ? (
              ...
              <div className="space-y-4">
                {Array.from(equipGroups.entries()).map(([eqName, items]) => (
                  <div key={eqName}>
                    <h3 className="text-sm font-display font-semibold text-secondary mb-2">{eqName}</h3>
                    <Table>
                      <TableHeader>
                        <TableRow>
