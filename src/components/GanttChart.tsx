                    ...
                    {row.tasks.map((task) =>
                      task.segments.map((seg, si) => {
                        ...
                        return (
                          ...
                          >
                            <span className="truncate leading-tight">{isOverflow ? `⚠ ${task.artigo}` : `${(task as unknown as MachineTask).isFirstPhase ? "1→ " : (task as unknown as MachineTask).isSequentialPhase ? "→ " : task.showSimultaneousBadge ? "⊗ " : ""}${task.artigo}`}</span>
                            {showTime && (
